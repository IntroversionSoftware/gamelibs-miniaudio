...
HEADERS = miniaudio.h research/miniaudio_engine.h
SOURCES = miniaudio_engine.c

HEADERS_INST := $(includedir)/miniaudio.h $(includedir)/miniaudio_engine.h
...
$(includedir)/miniaudio_engine.h: research/miniaudio_engine.h
	-@if [ ! -d $(includedir)  ]; then mkdir -p $(includedir); fi
	$(QUIET_INSTALL)cp $< $@
	@chmod 0644 $@
